<?php
	...
	try {
		...
		if($_SESSION['main']['role'] == 'group-admin'){
			...
			$emp = $con->prepare('SELECT * from emp_wages where company=:company');
			$emp->bindParam(':company', $company);
		} else {
			$emp = $con->prepare('SELECT * from emp_wages');
		}
	...
	<section class="content-header">
		<h1>Employee Wages</h1>
			<ol class="breadcrumb">
				...
				<li class="active">Employee Wages</li>
			</ol>
	</section>
			...
			<div class="col-sm-12">
				<div class="box box-info">
					<div class="box-header">
						<i class="fa fa-user-plus"></i> <h3 class="box-title">Employee Wages</h3>
						<!-- tools box -->
					...
					<div class="box-body">
						<div class="box">
						    <div class="box-header">
						     	...
						     	<div class="pull-right box-title"><a href="/hrms/admin/wages.php" class="">Add New</a></div>
						    </div>
						    <!-- /.box-header -->
						    <div class="box-body">
						            	...
						            	<?php 
						            		...
						            		foreach($empdata as $arcon){ 
												...
												if($empcount == 0){
							            			...
							            			?>
								            		<tr>
								            			<td><a href="/hrms/admin/wages.php?edit=<?=$arcon['id']?>"><?=$i?></a></td>
								            			<?php if($_SESSION['main']['role'] =='super-admin'){ ?>
															<td><?=$companyArray[$arcon['company']]?></td>
														<?php } ?>
								            			<td><?=$aremp['fname']?></td>
								            			<td><?=$aremp['lname']?></td>
								            			<td><?=$arcon['wage_group']?></td>
								            			<td><?=$arcon['type']?></td>
								            			<td><?=$arcon['wage']?></td>
								            			<td><?=$arcon['effective_date']?></td>
								            		</tr>
							            			<?php 
							            			$i++;
						            			} 

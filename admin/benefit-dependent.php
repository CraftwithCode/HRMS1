<?php
	...
	if(isset($_SESSION['main']['gid'])){
		$key = $_SESSION['main']['gid'].'_dependent';
	} else {
	...
	try {
		...
		if(isset($_GET['edit'])){
			$edit = $_GET['edit'];
			$emp = $con->prepare('SELECT * from benefit_dependent where id=:edit');
				...
				<div class="box box-info">
					...
					<div class="box-body">
						<form id="ben-links" name="ben_links" method="post" action="" class="benefit-steps">	
						  	<input type="hidden" name="formtype" value="benefit-dependent">
							...
							<div class="form-group" style="display: none;">
						  	...
						  	<div class="form-group">
						  		<label><?=$allFields['beneficiary']['title']?></label>
						  		<select name="beneficiary" class="form-control">
						  			<option value="Yes" <?=(isset($data) && $data['beneficiary']=="Yes")?'seleceted="seleceted"':''?>>Yes</option>
						  			<option value="No" <?=(isset($data) && $data['beneficiary']=="No")?'seleceted="seleceted"':''?>>No</option>
						  		</select>
						  	</div>
						  	<div class="form-group">
						  	...
						  	<div class="form-group">
						  		<label><?=$allFields['relation']['title']?></label>
						  		<?php $relationoptions = explode(',', $allFields['relation']['options']); ?>
						  		<select name="relationship" class="form-control">
						  			<?php foreach($relationoptions as $option){ ?>
						  				<option value="<?=$option?>" <?=(isset($data) && $data['relationship']==$option)?'seleceted="seleceted"':''?>><?=$option?></option>
						  			<?php } ?>
						  		</select>
						  	</div>
						  	<div class="form-group">
